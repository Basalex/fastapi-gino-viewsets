    ...
    def add_view(self, base_path: str = "", tags: list = None, **kwargs):
        def wrapper(view):
            ...
            if hasattr(view, "retrieve_list"):
                method = self.get(path=base_path, response_model=view.list_schema, tags=tags, **kwargs)
                method(view.retrieve_list)

            if hasattr(view, "retrieve"):
                method = self.get(path=path, response_model=view.output_schema, tags=tags, **kwargs)
                method(view.retrieve)

            if hasattr(view, "retrieve_single_object_data"):
                method = self.get(path=base_path, response_model=view.output_schema, tags=tags, **kwargs)
                method(view.retrieve_single_object_data)

            if hasattr(view, "create"):
                method = self.post(path=base_path, response_model=view.output_schema, tags=tags, **kwargs)
                method(view.create)

            if hasattr(view, "update"):
                view.update.__annotations__["request"] = view.get_put_schema()
                method = self.put(path=path, response_model=view.output_schema, tags=tags, **kwargs)
                method(view.update)

            if hasattr(view, "update_partial"):
                view.update_partial.__annotations__["request"] = view.get_patch_schema()
                method = self.patch(path=path, response_model=view.output_schema, tags=tags, **kwargs)
                method(view.update_partial)

            if hasattr(view, "delete"):
                method = self.delete(path=path, response_model=view.get_delete_schema(), tags=tags, **kwargs)
                method(view.delete)
